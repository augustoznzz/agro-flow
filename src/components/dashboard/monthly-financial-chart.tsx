...
function ChartBar({ 
    ...
    switch (type) {
      ...
      case 'balance':
        return {
          ...
          borderColor: data.balance >= 0 ? 'border-blue-600' : 'border-orange-600',
          icon: DollarSign,
          label: 'Saldo'
        }
    }
  ...
  return (
    <div className="flex flex-col items-center group relative">
      <div 
        className={`w-10 bg-gradient-to-t ${config.color} rounded-t-lg shadow-lg transition-all duration-500 hover:shadow-xl hover:scale-110 cursor-pointer relative overflow-visible`}
        style={{ 
          height: `${height}%`,
          minHeight: '8px'
        }}
      >
        ...
        {/* Tooltip */}
        <div className="absolute -top-20 left-1/2 transform -translate-x-1/2 opacity-0 group-hover:opacity-100 transition-all duration-300 ease-out z-50 pointer-events-none">
          <div className={`${config.bgColor} text-white text-xs font-semibold px-3 py-2 rounded-lg shadow-lg border-2 ${config.borderColor} whitespace-nowrap`}>
          ...
          {/* Seta do tooltip */}
          <div className={`absolute top-full left-1/2 transform -translate-x-1/2 w-0 h-0 border-l-4 border-r-4 border-t-4 border-transparent ${config.borderColor.replace('border-', 'border-t-')}`}></div>
        </div>
      </div>
    </div>
  )
            ...
            <div className="absolute inset-0 flex items-end justify-between px-4 pb-2">
              {monthlyData.map((data) => (
                <div key={data.month} className="flex flex-col items-center group relative h-full">
                  {/* Container das barras com maior espaçamento */}
